...
const News = () => {
  ...
  const fetchData = async () => {
    let resonse = await fetch(
      "https://newsapi.org/v2/everything?q=bitcoin&apiKey=d9c73c9d1c354205ae2ed5b79083e5e7"
    );
  ...
  return (
    <div key={ele.url} className="card" style={{ marginTop: "2rem", boxShadow: "rgba(0, 0, 0, 0.35) 0px 5px 15px" }}>
      ...
      <div className="card-body">
        ...
        {/* You can use the index here, for example: */}
        <p className="r" >Index: {index}</p>
      </div>
    </div>
  );
})}
